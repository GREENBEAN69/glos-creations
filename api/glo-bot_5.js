...
module.exports = async function handler(req, res) {
  ...
  // Optional customer context — only sent when customer is logged in AND asked
  // about their account (orders, wishlist, spending). This is a sensitivity
  // filter happening client-side so we don't waste tokens on every chat.
  var customerContext = (body.customerContext && typeof body.customerContext === 'object') ? body.customerContext : null;
  ...
  // Build a customer context appendix — included only when relevant.
  // Gives the AI knowledge of THIS specific customer's orders + wishlist so
  // it can answer "what did I buy?" or "what's in my wishlist?" naturally.
  var customerText = '';
  if (customerContext) {
    customerText = '\n\nLOGGED-IN CUSTOMER CONTEXT (use this to answer questions about THIS customer\'s account — never share with anyone else):\n';
    if (customerContext.firstName) customerText += '- Name: ' + customerContext.firstName + '\n';
    if (typeof customerContext.orderCount === 'number') {
      customerText += '- Total orders: ' + customerContext.orderCount + '\n';
    }
    if (typeof customerContext.lifetimeSpend === 'number' && customerContext.lifetimeSpend > 0) {
      customerText += '- Lifetime spend: $' + customerContext.lifetimeSpend.toFixed(2) + '\n';
    }
    if (Array.isArray(customerContext.orders) && customerContext.orders.length > 0) {
      customerText += '- Recent orders (most recent first):\n';
      for (var oi = 0; oi < customerContext.orders.length; oi++) {
        var ord = customerContext.orders[oi];
        var itemList = '';
        if (Array.isArray(ord.items) && ord.items.length > 0) {
          itemList = ord.items.map(function(it) {
            return it.title + (it.quantity > 1 ? ' (x' + it.quantity + ')' : '');
          }).join(', ');
        }
        customerText += '  • Order ' + ord.name + ' — placed ' + (ord.date || 'recently') + ' — Status: ' + ord.status + ' — Total: $' + (ord.total || 0).toFixed(2) + (itemList ? ' — Items: ' + itemList : '') + '\n';
      }
    }
    if (Array.isArray(customerContext.wishlist) && customerContext.wishlist.length > 0) {
      customerText += '- Wishlist (Saved for Later):\n';
      for (var wi = 0; wi < customerContext.wishlist.length; wi++) {
        var w = customerContext.wishlist[wi];
        customerText += '  • ' + w.name + ' ($' + (w.price || 0) + ')\n';
      }
    } else if (customerContext.orderCount === 0) {
      customerText += '- No orders yet — this is a brand new customer.\n';
    }
    customerText += '\nUse this info naturally when the customer asks about their account, but DO NOT proactively bring up purchase history unless asked. If they ask "what did I buy?" answer specifically using the orders above. If they ask about tracking or shipping address details, direct them to their account modal (👤 icon) → Orders tab → View Details for full info. NEVER mention shipping addresses, tracking numbers, or payment details — those are not in the context above for privacy reasons.';
  }
  ...
  try {
    var response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 500,
        system: SYSTEM_PROMPT + catalogText + customerText,
        messages: messages
      })
    });
